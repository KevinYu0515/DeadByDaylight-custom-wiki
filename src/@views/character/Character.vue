<template>
  <div class="records flex">
    <div class="leftInfor absolute left-0 top-0 flex flex-wrap justify-content-between flex-column z-5 w-auto h-screen">
        ...
        <ul class="select-items list-none mt-5">
          <li v-for="(item, index) in left_bar_items" :key="index" class="flex align-items-center justify-content-evenly py-1 cursor-pointer">
            <template v-if="currentPath !== `/records/${item.fullname}`">
              <img class="w-3rem" :src="require('@/assets/icon/IconHelp.png')" alt=""/>
            </template>
            <template v-else>
              <img class="w-3rem" :src="require('@/assets/icon/Loading_killer.png')" alt=""/>
            </template>
      ...
      <div class="px-3 w-9rem">
        <Button
          label="Other Killers"
          data-type="other-killers"  
          class="relative p-button-success my-2 w-full text-sm other-killers-button"
          @click="showOtherKillers = !showOtherKillers;"
        /> 
        <Button
          label="Back"
          data-type="back"  
          class="p-button-success my-2 w-full"
          @click="routerTo('/')"
        /> 
      </div>
    </div>
...
const character_id = computed(() => store.state.killers ? store.state.killers.data.character_id : "");
const killer = computed(() => store.state.killers ? store.state.killers.data.killersInfo.find(killer => killer.id === character_id.value) : null);
const other_killers = computed(() => store.state.killers ? store.state.killers.data.killersInfo.filter(killer => killer.id !== character_id.value) : []);
